...
A:人 @ bot 没循环风险——人下一次就走开了。bot @ bot 不刹会变成两台机器对着群里吐字数千轮。刹车设计:前两轮正常,第三轮系统提示词里塞个温和提示"差不多收尾吧",第四轮塞更强的,**第五轮起回复不带 @**——peer 那边的 mention 网关收不到 @ 事件就不会触发,链路自然死。**没有"完全不回复"那一档**,因为 OpenClaw SDK 不支持插件层 short-circuit 一次回复(`before_prompt_build` 的返回值里没有 skip 字段);drop @ 就是我们能做到的有效"刹车"。
